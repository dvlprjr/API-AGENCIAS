...
exports.Get = async (req, res) => {
    try {
        ...
        MSJ('Peticion usuarios ejecutada correctamente',  500, result, [], res, true);
        res.json(result);
    } catch (err) {
...
exports.GetByName = async (req, res) => {
    ...
    try {
        const result = await db.query(`
            ...
            WHERE PRFU00.PrfGCod IN ('ASESPYME', 'OFIPYME', 'GRTPYME', 'SUBGPYME', 'GERENTE')
            AND PRFU00.PrfUFecVto > GETDATE()
            AND RTRIM(FST746.Ubnom) LIKE :Nombre
        `, { 
            type: QueryTypes.SELECT,
            replacements: { Nombre: `%${Nombre}%` }
        });
        MSJ('Peticion usuarios ejecutada correctamente',  200, result, [], res,true);
        res.json(result);
    } catch (err) {
        console.error('Error en la consulta SQL:', err);
        res.status(500).send('Error al obtener datos: ' + err.message);
